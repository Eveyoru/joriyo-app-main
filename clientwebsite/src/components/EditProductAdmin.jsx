...
const EditProductAdmin = ({ close, data: propsData, fetchProductData }) => {
  const [data, setData] = useState({
    ...
    hasVariations: propsData.hasVariations || false,
    variations: propsData.variations || [],
    sizingType: propsData.sizingType || 'none'
  })
  ...
  const fetchVendors = async () => {
    try {
      ...
      if (responseData.success) {
        ...
        if (propsData.vendor && typeof propsData.vendor === 'object') {
          console.log("Product has vendor object:", propsData.vendor);
          setData(prev => ({
            ...prev,
            vendor: propsData.vendor._id
          }));
        } else if (propsData.vendor) {
          console.log("Product has vendor ID:", propsData.vendor);
          setData(prev => ({
            ...prev,
            vendor: propsData.vendor
          }));
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Vendor selected:", data.vendor)

    try {
      // Create a new object with the correct field name for product ID
      const submitData = {
        ...data,
        productId: data._id // Add productId field expected by the API
      }
      
      console.log("Submitting with data structure:", submitData)
      ...
      if (responseData.success) {
        console.log("Updated product response:", responseData.data)
        console.log("Updated product vendor:", responseData.data.vendor)
  ...
  const handleVendorSelect = (e) => {
    ...
    console.log("Setting vendor to:", vendorId || null);
            ...
            <form className='grid gap-4' onSubmit={handleSubmit}>
              ...
              <div className='grid gap-1'>
                ...
                <div>
                  ...
                  {data.vendor && (
                    <div className='flex items-center gap-1 mt-1 bg-blue-50 py-1 px-3 rounded w-fit'>
                      <p className='text-sm'>
                        {vendors.find(v => v._id === data.vendor)?.name || (typeof data.vendor === 'object' ? data.vendor.name : "Loading vendor...")}
                      </p>
                      <button
                        type='button'
