...
const UploadFeaturedCategoryModel = ({ close, fetchData }) => {
    ...
    const getNextDisplayOrder = async () => {
        try {
            ...
            const maxOrder = Math.max(...featuredCategories.map(cat => cat.displayOrder), -1);
            return maxOrder + 1;
        } catch (error) {
            return 0;
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const response = await Axios({
                ...SummaryApi.addFeaturedCategory,
                data: data
            });
        ...
        <section className='fixed top-0 bottom-0 left-0 right-0 p-4 bg-neutral-800 bg-opacity-60 flex items-center justify-center z-50'>
            <div className='bg-white rounded-lg p-5 max-w-2xl w-full max-h-[90vh] overflow-y-auto'>
                <div className='flex items-center justify-between mb-2'>
                    <h2 className='font-semibold text-lg'>Add Featured Category</h2>
                ...
                <form className='my-3 grid gap-4' onSubmit={handleSubmit}>
                    ...
                    <button
                        ...
                        `}
                    >
                        {loading ? <Loading /> : "Add Featured Category"}
                    </button>
                </form>
            </div>
        </section>
...
export default UploadFeaturedCategoryModel; 